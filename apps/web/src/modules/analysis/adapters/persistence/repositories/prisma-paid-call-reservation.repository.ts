  ...
  async lockByCallId(
    ...
    const row = tx
      ? (
          ...
          >(
            Prisma.sql`SELECT id, project_id, analysis_run_id, call_id, request_hash, worst_case_microusd, provider_response_id, usage_event_id, status FROM paid_call_reservations WHERE call_id = CAST(${callId} AS uuid) FOR UPDATE`,
          )
        )[0]
    ...
    if ('project_id' in row)
      return {
        ...
        worstCaseMicrousd: row.worst_case_microusd,
    ...
    return {
      ...
      worstCaseMicrousd: row.worstCaseMicrousd,
