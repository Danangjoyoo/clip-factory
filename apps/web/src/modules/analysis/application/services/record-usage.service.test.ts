...
describe('RecordUsageService', () => {
  it('rejects a completed reservation with a different provider response', async () => {
    const service = new RecordUsageService(
      ...
      new AIUsageEventDataService({
        findByProviderResponseId: async () => null,
        insert: async () => {
          throw new Error('not reached');
        },
      }),
      new AnalysisRunDataService({
        findById: async () => null,
        addActualCost: async () => undefined,
        addUncertain: async () => undefined,
        reconcileUncertain: async () => undefined,
      }),
  ...
  it('reconciles an uncertain reservation using its locked worst-case amount', async () => {
    ...
    const service = new RecordUsageService(
      ...
      new AIUsageEventDataService({
        findByProviderResponseId: async () => null,
        insert: async (value) => ({ id: 'event', ...value }) as never,
      }),
      new AnalysisRunDataService({
        findById: async () => null,
        addActualCost: async () => undefined,
        addUncertain: async () => undefined,
        reconcileUncertain: async (_id, amount) => {
          reconciled = amount;
        },
      }),
      new PaidCallReservationDataService({
