  ...
  async createManual(
    ...
    const row = await prisma.clip.create({
      data: {
        ...
        captionJson: input.captionDocument as Prisma.InputJsonValue,
        styleJson: input.style as Prisma.InputJsonValue,
        frameJson: input.frame as Prisma.InputJsonValue,
        state: 'CANDIDATE',
      },
    });
    return clipRecordToEntity({
      ...row,
      origin: row.origin,
      state: row.state,
      captionJson: row.captionJson,
      scoreJson: row.scoreJson,
      styleJson: row.styleJson,
      frameJson: row.frameJson,
    } as never);
  }
}
