...
describe('AddManualClipService', () => {
  it.each(['MANUAL', 'AI_HIGHLIGHTS'] as const)(
    'adds a zero-cost manual clip to %s project',
    async (mode) => {
      const h = manualClipHarness({
        mode,
      ...
      expect(
        result.captionDocument.cues
          .flatMap((cue) => cue.words)
          .map((word) => word.text),
      ).toEqual(['first', 'complete', 'thought']);
      expect(h.preparation.calls).toEqual([
        { clipId: result.id, startMs: 10000, endMs: 40000 },
      ]);
