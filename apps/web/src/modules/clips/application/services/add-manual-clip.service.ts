...
export class AddManualClipService {
  constructor(
    ...
    private readonly captions = new BuildCaptionDocumentService({
  ...
  async execute(input: AddManualClipEntityRequest): Promise<ClipEntityDto> {
    ...
    const duration =
      ...
      'durationMs' in source.probe
        ? Number(source.probe.durationMs)
        : 0;
    if (!Number.isFinite(duration) || duration <= 0)
      throw new ClipError('SOURCE_NOT_PROBED');
    let startMs: number, endMs: number;
    try {
      startMs = parseTimecode(input.startTimecode);
      endMs = parseTimecode(input.endTimecode);
    } catch (error) {
      if (error instanceof ClipEditError) throw new ClipError(error.code);
      throw error;
    }
    if (endMs <= startMs) throw new ClipError('CLIP_END_NOT_AFTER_START');
    if (endMs > duration) throw new ClipError('CLIP_OUTSIDE_SOURCE');
    if (endMs - startMs > project.defaultMaxClipSeconds * 1000)
      throw new ClipError('CLIP_TOO_LONG');
    const words = await this.transcripts.wordsInRange(
      project.id,
      startMs,
      endMs,
    );
    if (!words.length) throw new ClipError('TRANSCRIPT_NOT_FOUND');
    const captionDocument = this.captions.execute({
      languageTag: project.languageTag,
      words,
      startMs,
      endMs,
    });
    const clip = await this.clips.createManual(
      {
    ...
    await this.preparation.prepare({
      projectWorkflowId: project.activeWorkflowId ?? '',
      clipId: clip.id,
      startMs,
      endMs,
    });
