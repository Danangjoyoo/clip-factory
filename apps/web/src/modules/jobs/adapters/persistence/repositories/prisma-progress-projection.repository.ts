...
const activeStatuses = ['QUEUED', 'RUNNING', 'WAITING', 'WORKER_OFFLINE'] as const;
...
export class PrismaProgressProjectionRepository implements ProgressProjectionRepository {
  ...
  async upsert(event: ProgressPresentation) {
    ...
    const status = (event.status ?? 'RUNNING') as (typeof activeStatuses)[number];
    const data = {
    ...
    await this.db.jobProjection.upsert({
      where: { workflowId_runId: { workflowId: event.workflowId, runId: event.workflowId } },
  ...
  async findActive(projectId: string): Promise<ProgressPresentation[]> {
    ...
    return rows.map((row) => ({
      ...
      eta: { lowSeconds: row.etaLowSeconds, highSeconds: row.etaHighSeconds, confidence: 'LOW' as const },
      completedUnits: row.progressBasisPoints,
      totalUnits: 10000,
      unit: 'BASIS_POINTS',
      occurredAt: (row.lastHeartbeatAt ?? row.updatedAt).toISOString(),
      status: row.status,
      ...(row.lastHeartbeatAt ? { heartbeatAt: row.lastHeartbeatAt.toISOString() } : {}),
    }));
  }

  private async isNewer(workflowId: string, occurredAt: Date) {
    const row = await this.db.jobProjection.findFirst({ where: { workflowId } });
    if (row && ['COMPLETED', 'FAILED', 'CANCELLED'].includes(row.status)) return false;
