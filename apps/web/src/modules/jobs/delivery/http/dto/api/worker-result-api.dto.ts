import { z } from 'zod';
const ObjectReference = z.object({ bucket: z.string().min(1), key: z.string().min(1), versionId: z.string().min(1).nullable(), sha256: z.string().regex(/^[a-f0-9]{64}$/) }).strict();
export const WorkerResultApiSchema = z
  .object({
    ...
    error: z.object({ code: z.string().min(1), message: z.string().min(1) }).strict().nullable().optional(),
    uncertainReservedMicrousd: z.string().regex(/^[0-9]+$/).optional(),
