...
export class RelinkSourceService {
  constructor(private readonly uow: UnitOfWork, private readonly sources: SourceAssetDataService, private readonly workflows: WorkflowControlPort, private readonly validator: SourceValidationPort) {}
  async execute(input: { projectId: string; candidate: Candidate; confirmedFingerprint?: string }) {
    ...
    const validated = await this.validator.validateCandidate({ sourceAssetId: source.id, candidatePath: input.candidate.resolvedPath });
    if (!compatible(source.probe, validated.probe)) throw new RelinkIncompatibleError('candidate media is incompatible');
    if (source.fingerprint !== validated.fingerprint && input.confirmedFingerprint !== validated.fingerprint) return { sourceAssetId: source.id, confirmationRequired: true, fingerprint: validated.fingerprint };
    const updated = await this.uow.execute(async (tx) => this.sources.relink(source.id, { ...validated, health: 'LOCATED' }, tx));
