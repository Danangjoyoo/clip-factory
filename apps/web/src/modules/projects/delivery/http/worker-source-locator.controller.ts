...
export class WorkerSourceLocatorController {
  ...
  async get(request: Request, id: string) {
    ...
    if (!UUID.test(id)) return Response.json({ code: 'INVALID_SOURCE_ID' }, { status: 422 });
    try {
      ...
      if (error instanceof Error && error.message.startsWith('Source not ready'))
  ...
  async validate(request: Request, sourceAssetId: string) {
    if (
      ...
      return Response.json({ code: 'INTERNAL_UNAUTHORIZED' }, { status: 401 });
    if (!UUID.test(sourceAssetId)) return Response.json({ code: 'INVALID_SOURCE_ID' }, { status: 422 });
    ...
    if (
      ...
      return Response.json({ code: 'INVALID_IDEMPOTENCY_KEY' }, { status: 422 });
    if (parsed.data.sourceAssetId !== sourceAssetId)
      return Response.json({ code: 'INVALID_SOURCE_VALIDATION' }, { status: 422 });
