...
export class ExportDiagnosticsService {
  constructor(private readonly archive: DiagnosticsArchive) {}
  execute(records: Record<string, unknown>[]) {
    return this.archive.create(records.map(redactDiagnosticRecord));
  }
}
