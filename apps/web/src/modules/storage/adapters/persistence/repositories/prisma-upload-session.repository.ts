...
export class PrismaUploadSessionRepository implements UploadSessionRepository {
  private db(tx?: TransactionContext): Db['uploadSession'] {
    return (
      (tx as { uploadSession?: Db['uploadSession'] } | undefined)
  ...
  private entity(row: {
    ...
    return {
      ...
      status,
      completedPartsHash: row.completionPartsHash,
      objectReference: row.objectSha256
        ? {
            key: row.objectKey,
  ...
  async create(session: UploadSessionEntityDto, tx?: TransactionContext) {
    const row = await this.db(tx).create({
      data: {
        id: session.id,
  ...
  async requireOwned(
    ...
    const row = await this.db(tx).findFirst({
  ...
  async markCompleted(
    ...
    const row = await this.db(tx).update({
      where: { id: sessionId },
      data: {
        status: 'COMPLETED',
