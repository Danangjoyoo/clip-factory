...
export class UploadSessionDataService {
  ...
  create(
    session: import('../dto/entity/upload-session-entity.dto').UploadSessionEntityDto,
    tx?: TransactionContext,
  ) {
    return this.repository.create(session, tx);
  }
  requireOwned(sessionId: string, projectId: string, tx?: TransactionContext) {
    return this.repository.requireOwned(sessionId, projectId, tx);
  }
  markCompleted(
    sessionId: string,
  ...
  markAborted(sessionId: string, tx?: TransactionContext) {
    return this.repository.markAborted(sessionId, tx);
  }
}
