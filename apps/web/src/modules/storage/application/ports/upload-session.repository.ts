...
export interface UploadSessionRepository {
  create(session: UploadSessionEntityDto, transaction?: TransactionContext): Promise<UploadSessionEntityDto>;
  requireOwned(sessionId: string, projectId: string, transaction?: TransactionContext): Promise<UploadSessionEntityDto>;
  markCompleted(sessionId: string, reference: ImmutableObjectReference, transaction: TransactionContext, completionPartsHash?: string): Promise<UploadSessionEntityDto>;
