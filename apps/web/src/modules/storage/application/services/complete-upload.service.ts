...
} from '../ports/artifact-store.port';
import type {
  MultipartUploadPort,
  CompletedPart,
} from '../ports/multipart-upload.port';
...
export class CompleteUploadService {
  ...
  async execute(
    ...
    const uploadedParts = await this.multipart.listParts(
      session.objectKey,
      session.uploadId,
    );
    ...
    if (
      parts.some(
        (part) => uploadedByNumber.get(part.partNumber)?.etag !== part.etag,
      )
    )
      throw new UploadError('INVALID_PART');
    const completed = await this.multipart.complete(
      session.objectKey,
      session.uploadId,
      parts,
    );
    ...
    if (
      completed.versionId !== null &&
      head.versionId !== null &&
      completed.versionId !== head.versionId
    ) {
      await this.artifacts.deleteMany([session.objectKey]);
      throw new UploadError('INVALID_VERSION');
    }
    ...
    const result = await this.uow.execute(async (tx) => {
      const updated = await this.sessions.markCompleted(
        session.id,
        reference,
        tx,
        partsHash,
      );
      await this.sources.attachUploadedObject(input.projectId, reference, tx);
      return updated;
    });
