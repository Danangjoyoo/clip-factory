...
export class CompleteUploadService {
  ...
  async execute(input: Readonly<{ projectId: string; sessionId: string; parts: readonly CompletedPart[] }>) {
    ...
    if (session.status === 'COMPLETED') throw new UploadError('UPLOAD_ALREADY_COMPLETED_CONFLICT');
    ...
    const result = await this.uow.execute(async (tx) => { const updated = await this.sessions.markCompleted(session.id, reference, tx); await this.sources.attachUploadedObject(input.projectId, reference, tx); return updated; });
    return { session: result, reference, partsHash: createHash('sha256').update(JSON.stringify(parts, (_, value) => typeof value === 'bigint' ? value.toString() : value)).digest('hex') } as const;
  }
}
