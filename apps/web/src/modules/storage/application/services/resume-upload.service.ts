...
import { assertNotExpired } from './upload-policy';
export class ResumeUploadService {
  ...
  async execute(input: Readonly<{ projectId: string; sessionId: string; totalParts: number }>) {
    ...
    for (let partNumber = 1; partNumber <= input.totalParts; partNumber += 1) if (!completedNumbers.has(partNumber)) parts.push({ partNumber, url: await this.multipart.presignPart(session.objectKey, session.uploadId, partNumber, 900), expiresSeconds: 900 });
