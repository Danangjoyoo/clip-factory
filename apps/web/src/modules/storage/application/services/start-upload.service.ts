...
export class StartUploadService {
  constructor(private readonly sessions: UploadSessionDataService, private readonly multipart: MultipartUploadPort, private readonly sources?: SourceAssetDataService) {}
  async execute(input: Readonly<{ projectId: string; sourceAssetId: string; fileName: string; contentType: string; sizeBytes: bigint; totalParts: number; }>) {
    ...
    if (this.sources) {
      ...
      if (!source || source.projectId !== input.projectId || source.kind !== 'BROWSER_UPLOAD') throw new Error('SOURCE_NOT_OWNED');
    }
    ...
    const now = new Date();
    const session: UploadSessionEntityDto = { id: sessionId, projectId: input.projectId, sourceAssetId: input.sourceAssetId, kind: 'BROWSER_UPLOAD', objectKey: key, uploadId, fileName: input.fileName, contentType: input.contentType, declaredSizeBytes: input.sizeBytes, totalParts: input.totalParts, status: 'ACTIVE', completedPartsHash: null, objectReference: null, expiresAt: new Date(now.getTime() + 24 * 60 * 60 * 1000), createdAt: now, updatedAt: now };
