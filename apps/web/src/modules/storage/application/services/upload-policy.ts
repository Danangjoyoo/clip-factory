export type UploadErrorCode = 'INVALID_PART' | 'TOO_MANY_PARTS' | 'INVALID_FILENAME' | 'SOURCE_TOO_LARGE' | 'UPLOAD_SIZE_MISMATCH' | 'UPLOAD_EXPIRED' | 'UPLOAD_ALREADY_COMPLETED_CONFLICT' | 'INVALID_SHA256' | 'INVALID_VERSION';
...
export function sourceObjectKey(projectId: string, sessionId: string, fileName: string): string {
  ...
  if (!base || base !== fileName || !EXTENSIONS.has(extension)) throw new UploadError('INVALID_FILENAME');
...
export function assertPart(partNumber: number): void {
  if (!Number.isInteger(partNumber) || partNumber < 1 || partNumber > 10000) throw new UploadError('INVALID_PART');
}
export function assertNotExpired(expiresAt: Date): void { if (expiresAt.getTime() <= Date.now()) throw new UploadError('UPLOAD_EXPIRED'); }
