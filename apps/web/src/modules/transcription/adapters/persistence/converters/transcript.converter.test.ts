...
import {
  transcriptEntityToRecord,
  transcriptRecordToEntity,
} from './transcript.converter';
...
describe('transcript mapping', () =>
  it('round trips provenance without casts', () =>
    expect(
      transcriptRecordToEntity({
        ...transcriptEntityToRecord(entity),
        id: entity.id,
...
describe('transcript provenance validation', () => {
  ...
  it('rejects MLX provenance that is not the pinned manifest', () => {
    ...
    ).toThrow('INVALID_TRANSCRIPT_MODEL_PROVENANCE');
  });
});
