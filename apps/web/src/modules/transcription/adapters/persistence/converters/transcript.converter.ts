...
const MLX_WEIGHTS_SHA256 = '05ff791ce3630fae47e7c51004e9666204d786246ec07cac6110af768099b40d';
const SHA256 = /^[a-f0-9]{64}$/;

export const transcriptRecordToEntity = (r: TranscriptRecordDto): TranscriptEntityDto => {
...
function validateEntity(e: Omit<TranscriptEntityDto, 'id' | 'createdAt'>): Omit<TranscriptRecordDto, 'id' | 'createdAt'> {
...
function validateTranscript(value: Pick<TranscriptRecordDto, 'backend' | 'model' | 'modelRevision' | 'weightsSha256' | 'objectBucket' | 'objectKey' | 'objectVersionId' | 'objectSha256'>): void {
  const backend = assertBackend(value.backend);
  if (!value.model || !value.modelRevision || !value.objectBucket || !value.objectKey || !value.objectVersionId) {
    throw new Error('INVALID_TRANSCRIPT_PROVENANCE');
  }
  if (!SHA256.test(value.objectSha256)) throw new Error('INVALID_TRANSCRIPT_ARTIFACT_HASH');
  if (backend === 'FAKE' && value.weightsSha256 !== null) throw new Error('INVALID_TRANSCRIPT_WEIGHTS_HASH');
  if (backend === 'MLX_WHISPER' && (value.modelRevision !== MLX_MODEL_REVISION || value.weightsSha256 !== MLX_WEIGHTS_SHA256)) {
    throw new Error('INVALID_TRANSCRIPT_MODEL_PROVENANCE');
  }
  if (value.weightsSha256 !== null && !SHA256.test(value.weightsSha256)) throw new Error('INVALID_TRANSCRIPT_WEIGHTS_HASH');
}
