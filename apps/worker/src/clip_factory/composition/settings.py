...
class WorkerSettings:
    ...
    def audio_validation_receipts(self) -> AudioValidationReceiptPort:
        from clip_factory.adapters.storage.json_audio_validation_receipt_store import JsonAudioValidationReceiptStore
    ...
    def from_mapping(cls, values: Mapping[str, str]) -> 'WorkerSettings':
        ...
        return cls(cast(Literal['fake', 'live'], adapter), SecretStr(api_key) if api_key else None, SecretStr(token), values.get('INTERNAL_API_BASE_URL', 'http://127.0.0.1:3000'), values.get('TEMPORAL_ADDRESS', '127.0.0.1:7233'), values.get('REDIS_URL', 'redis://127.0.0.1:6379/0'), values.get('MINIO_ENDPOINT', 'http://127.0.0.1:9000'), SecretStr(values.get('MINIO_ACCESS_KEY', 'clip_factory_local')), SecretStr(values.get('MINIO_SECRET_KEY', 'clip_factory_local_secret')), roots, Path(values.get('AUDIO_VALIDATION_RECEIPT_PATH', './data/audio-validation-receipts.json')).expanduser())
