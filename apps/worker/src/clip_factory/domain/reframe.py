...
class SubjectObservation:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("center_x_micros", "center_y_micros", "confidence_micros", "speaking_micros", "area_micros", "center_proximity_micros"):
...
def build_crop_track(observations: tuple[SubjectObservation, ...] | list[SubjectObservation], alpha_micros: int = 250_000, confidence_floor: int = 500_000) -> tuple[CropPoint, ...]:
...
def apply_manual_focal_point(track: tuple[CropPoint, ...], x_micros: int, y_micros: int) -> tuple[CropPoint, ...]:
    if not 0 <= x_micros <= MICROS or not 0 <= y_micros <= MICROS:
        raise ValueError("focal point outside fixed-point range")
    return tuple(CropPoint(point.time_ms, x_micros, y_micros, MICROS, "MANUAL_FOCAL_POINT") for point in track)
