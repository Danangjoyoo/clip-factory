...
async def reconcile_paid_call(
    ...
    if reconcile is not None:
        ...
        if isinstance(result, HighlightResponse):
            return result
    return None
...
async def call_openai_once(
    ...
    mark_sent = getattr(deps, "mark_sent", None)
    if mark_sent is not None:
        try:
            await mark_sent(call.call_id, call.request_hash)
        except Exception as exc:
    ...
    try:
        ...
        raise PaidCallUncertainError("provider outcome is uncertain") from exc
    await deps.put_json(
        f"projects/{call.project_id}/analysis/{call.analysis_run_id}/calls/{call.call_id}.json",
        {
            "callId": call.call_id,
            "requestHash": call.request_hash,
            "providerResponseId": response.response_id or "",
            "normalizedUsage": response.usage or {},
            "validatedResponse": {"candidateCount": len(response.candidates)},
        },
    )
    return response
