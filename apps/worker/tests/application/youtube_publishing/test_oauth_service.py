...
def test_process_memory_flow_store_pop_is_one_time_and_has_no_serialization_method() -> (
    None
):
...
def test_begin_stores_only_digest_then_opens_system_browser() -> None:
    ...
    assert (
        result.authorization_display_url
...
def test_begin_rejects_non_exact_loopback_callback_uri(redirect_uri: str) -> None:
    fakes = make_oauth_fakes(
        now=datetime(2026, 7, 11, tzinfo=UTC), redirect_uri=redirect_uri
    )
    service = YouTubeOAuthService(**fakes.dependencies)
...
def test_complete_consumes_state_once_validates_scope_and_reports_sanitized_connection() -> (
    None
):
    fakes = make_oauth_fakes(now=datetime(2026, 7, 11, tzinfo=UTC))
    service = YouTubeOAuthService(**fakes.dependencies)
...
def test_complete_rejects_denial_mismatch_expiry_and_bad_scope() -> None:
    ...
    with pytest.raises(OAuthSecurityError, match="authorization denied"):
        asyncio.run(
            service.complete(
                OAuthCallback(
                    "127.0.0.1", "/oauth2/callback", state, None, "access_denied"
                )
            )
        )
    asyncio.run(service.begin(CONNECTION_ID))
    with pytest.raises(OAuthSecurityError, match="missing or already consumed"):
        asyncio.run(
            service.complete(
                OAuthCallback("127.0.0.1", "/oauth2/callback", "wrong", "code")
            )
        )
    asyncio.run(service.begin(CONNECTION_ID))
    ...
    with pytest.raises(OAuthSecurityError, match="missing or already consumed"):
        asyncio.run(
            service.complete(
                OAuthCallback("127.0.0.1", "/oauth2/callback", state, "code")
            )
        )
