...
def test_source_relink_and_manual_clip_are_canonical_transitions() -> None:
    assert transition(JobState.VALIDATING_SOURCE, "source_missing") is JobState.SOURCE_MISSING
    ...
    assert transition(JobState.AWAITING_REVIEW, "manual_clip") is JobState.AWAITING_REVIEW
    with pytest.raises(ValueError):
