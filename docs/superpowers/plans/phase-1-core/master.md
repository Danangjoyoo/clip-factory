...
- Strict witnessed RED → GREEN → REFACTOR is mandatory. A worker writes no production code until the exact relevant test has been run and failed for the expected missing behavior. If production code is written first, delete it and restart that cycle.
...
- Controllers validate and map transport concerns, then call one application entry point. Services own policy and orchestration. Each data service imports exactly one application-owned repository port. Each concrete repository owns one table/entity and accepts/returns Record DTOs only.
- API Schema DTOs, Entity DTOs, Record DTOs, Client Schema DTOs, provider models, Temporal payloads, and UI presentation models remain distinct. Enums are boundary-owned. Only API↔Entity, Entity↔Record, and Client↔Entity converters are allowed, and each converter has direct tests.
...
TypeScript tests are colocated as `*.test.ts` or `*.test.tsx`; Python tests mirror `src/clip_factory` below `apps/worker/tests`; cross-system tests live under `tests/integration`, `tests/media`, and `tests/e2e`. Contract source is `packages/contracts/schema/*.schema.json`, generated TypeScript is `packages/contracts/src/generated/`, and generated Python is `apps/worker/src/clip_factory/entrypoints/contracts/generated/`.
