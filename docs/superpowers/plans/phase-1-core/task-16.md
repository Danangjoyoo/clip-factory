...
- Create: `apps/web/src/modules/analysis/converters/entity-record/ai-usage-event.converter.ts`
- Create: `apps/web/src/modules/analysis/converters/entity-record/cost-allocation.converter.ts`
...
- Test: `apps/web/src/modules/analysis/converters/entity-record/ai-usage-event.converter.test.ts`
- Test: `apps/web/src/modules/analysis/converters/entity-record/cost-allocation.converter.test.ts`
...
- [ ] **RED: exact cost and immutable event test.** A callback for response `resp_001` with 1M input and 100k output prices to 8,000,000 micro-USD; repeating identical callback returns same event; changed usage under same response ID throws `PAID_CALL_CONFLICT`.

- [ ] Run `pnpm exec vitest run apps/web/src/modules/analysis/application/services/record-usage.service.test.ts`; expect import FAIL.
...
export class RecordUsageService {
  ...
  async execute(input: UsageEntityInput): Promise<AIUsageEventEntityDto> {
    return this.unitOfWork.execute(async (transaction) => {
      ...
      if (existing) {
        if (existing.requestHash !== input.requestHash) throw new AnalysisError('PAID_CALL_CONFLICT');
        return existing;
      }
      ...
      await this.reservations.complete(input.callId, input.providerResponseId, transaction);
...
- [ ] Run usage test; expect PASS. Add a `FailAfterEventInsertUnitOfWork` test that throws between event insertion and reservation completion; assert no event, reservation update, or analysis total survives, then retry and assert one of each. Add converter tests for every token category, all reasoning enum values, pricing tier, UTC timestamp, response ID, purpose, project/run/clip IDs, and bigint↔decimal string.
...
- [ ] Run allocation test; expect PASS and assert sum equals total for 0–20 candidates and totals 0–1000.
