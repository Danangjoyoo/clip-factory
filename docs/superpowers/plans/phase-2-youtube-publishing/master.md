...
- **Recovery state policy:** The generic publication transition policy never permits `UPLOAD_OUTCOME_UNCERTAIN -> UPLOADING`, `UPLOAD_OUTCOME_UNCERTAIN -> YOUTUBE_PROCESSING`, or `FAILED -> READY_TO_UPLOAD`. Only dedicated evidence-gated functions acting on one locked durable publication/attempt snapshot may authorize an ordinary pre-final retry, an acknowledged unresolved replacement, or a reconciled `VIDEO_FOUND` continuation; request/UI booleans cannot substitute for final-dispatch, reconciliation, or acknowledgement evidence.
...
- **Persistence ownership:** Next.js is the only PostgreSQL writer. Application repository ports are Entity-oriented; each data service imports exactly one matching port and no adapter. Prisma repositories own Record DTOs and Entity↔Record conversion internally, implement one port/table each, and are injected only by composition. Record/Prisma types never appear in application interfaces. Cross-table transactions, approval, schedule, idempotency, and upload decisions stay in application services.
