...
| 65 | How can the native worker validate/use a source without reading PostgreSQL or leaking its locator into Temporal history? | PostgreSQL initially stores the submitted local display path with null `resolvedPath`. Service-authenticated internal locator read/update endpoints let a native activity fetch the submitted/resolved path or MinIO reference and atomically persist validation. The activity/adapter consumes it in memory (materializing uploaded objects only to a private temp/cache path) and returns only sanitized probes, hashes, IDs, and permitted object references; locators are absent from public APIs, workflow payloads/history, logs, and diagnostics. |
