...
| 65 | How can the native worker use a local path without reading PostgreSQL or leaking it into Temporal history? | A service-authenticated internal locator endpoint maps `SourceAsset` ID to the local path or MinIO reference inside an activity. The activity consumes the locator in memory and returns only sanitized probes, hashes, IDs, and permitted object references; the locator is absent from public APIs, workflow payloads/history, logs, and diagnostics. |
