...
| 65 | How can the native worker validate/use a source without reading PostgreSQL or leaking its locator into Temporal history? | PostgreSQL initially stores the submitted local display path with null `resolvedPath`. Service-authenticated internal locator read/update endpoints let native activities fetch the submitted/resolved path or MinIO reference. Locator validation atomically stores path metadata/fingerprint and marks `LOCATED`; media validation later stores the probe and marks `HEALTHY`. The activity/adapter consumes locators in memory (materializing uploaded objects only to a private temp/cache path) and returns only sanitized probes, hashes, IDs, and permitted object references; locators are absent from public APIs, workflow payloads/history, logs, and diagnostics. |
| 66 | How is the local transcription model made reproducible? | Cache `mlx-community/whisper-large-v3-mlx` at immutable revision `49e6aa286ad60c14352c404340ded53710378a11`, verify the reviewed `weights.npz` SHA-256 `05ff791ce3630fae47e7c51004e9666204d786246ec07cac6110af768099b40d`, pass its local directory to MLX Whisper, and record that revision/hash in transcript provenance. Never record mutable `main@date` as a revision. |
| 67 | What if the approved GPT-5.6 default is not enabled for the user's API project? | Keep `gpt-5.6-sol`/high as the quality default, add `gpt-5.5` as an explicit selectable fallback, and use a non-inference model-access check after key configuration. Disable/labeled unavailable options and never switch model or reasoning silently. Credits alone are not treated as proof of GPT-5.6 entitlement. |
| 68 | How does the MVP avoid surprise GPT-5.6 prompt-cache write charges? | Send GPT-5.6 requests in explicit prompt-cache mode with no breakpoints, which disables cache reads/writes for that request. Keep cache-write pricing/usage fields in versioned provenance for a future opt-in caching strategy, but do not enable it silently. |
